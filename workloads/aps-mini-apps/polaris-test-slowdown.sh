...
recover_interval=1
sinograms_per_task=1
tasks_per_sirt=2
slowdown_selections=(1 2 3)
...
workflow=execute-pipeline-with-slowdown.py
...
for num_sirt in "${num_sirts[@]}"; do
  for slowdown_selection in "${slowdown_selections[@]}"; do
    ...
    WORKSPACE="$TOP/num_sirt-$num_sirt-slowdown-$slowdown_selection"
    mkdir -p "$WORKSPACE"
    cd "$WORKSPACE"
    ...
    num_task=$(( num_sirt * tasks_per_sirt ))
    ...
    json_set "$PARAMS_FILE" "slowdown" "$slowdown_selection"
    # Examples:
