...
class SirtOperator(FlatMapFunction):
    ...
    def open(self, ctx: RuntimeContext):
        ...
        # Load slowdown
        input_file = os.getcwd() + "/slow_down_samples.txt"
        # Load slowdown intervals, the intervals is generated by `slowdown-gen.py`
        slowdown = np.loadtxt(input_file, dtype=np.int64)
        print(f"[SirtOperator.open] Loaded slowdown from: {input_file}", slowdown.shape)
        print("[SirtOperator.open] Dtype:", slowdown.dtype)
        # Example usage
        print("[SirtOperator.open] Mean of set 1:", slowdown[1].mean())
        print("[SirtOperator.open] First value:", slowdown[1, 0])
        self.slowdown_time = slowdown[self.args.slowdown, self.task_id]
        print(f"[SirtOperator.open] SlowdownIndex={self.task_id} --> slowdown: {self.slowdown_time} --> adjusted for data per iteration (4): {self.slowdown_time/4}")
        self.slowdown_time /= 4
